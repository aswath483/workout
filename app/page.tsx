...
export default function App() {
  ...
  const [completedWeeks, setCompletedWeeks, weeksLoaded] = useLocalStorage<number[]>('wk_weeks', []);
  ...
  const [completedDays, setCompletedDays, daysLoaded] = useLocalStorage<string[]>('wk_days_done', []);
  ...
  const handleDayComplete = (dayKey: string) => {
    if (completedDays.includes(dayKey)) return;
    setCompletedDays((prev) => [...prev, dayKey]);
    // Update streak
  ...
  if (!checkedLoaded || !weeksLoaded || !levelLoaded || !streakLoaded || !daysLoaded) {
    return (
          ...
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-t-3xl p-6 w-full max-w-lg"
            ...
            <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl p-4 mb-5">
              ...
              <p className="text-[#ccc] text-sm leading-relaxed">
                After you finish <span className="text-white font-semibold">Set 1</span> of an exercise, you take a break before doing <span className="text-white font-semibold">Set 2</span>. That break is called <span className="text-white font-semibold">rest between sets</span>.
                The timer starts automatically when you tap a set button.
              </p>
              <div className="mt-3 space-y-1 text-xs text-[#888]">
                <p><span className="text-[#4ade80] font-semibold">Beginner:</span> Big lifts (Squat/Deadlift) → 3 min rest. Smaller exercises → 2 min rest.</p>
